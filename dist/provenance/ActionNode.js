 ...
 */
import { GraphNode, AttributeContainer, GraphEdge, GraphUtils } from '../graph/graph';
...
export class ActionNode extends GraphNode {
    ...
    get resultsIn() {
        const r = this.outgoing.filter(GraphUtils.isType('resultsIn'))[0];
        return r ? r.target : null;
    }
    get previous() {
        const r = this.incoming.filter(GraphUtils.isType('next'))[0];
        return r ? r.source : null;
    }
}
